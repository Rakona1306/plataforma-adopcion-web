...
{
    public class CreateGivingValidator : AbstractValidator<CreateGivingDto>
    {
        public CreateGivingValidator()
        {
            RuleFor(x => x.Name)
            ...
            // Reglas condicionales según el tipo de donación (MONEY vs GOODS)
            When(x => x.Type == GivingType.MONEY, () =>
            {
                ...
                RuleFor(x => x.Quantity).Null().WithMessage("La cantidad no debe llenarse para dinero.");
