...
export default function AdoptionPageNext() {
    ...
    const { data, isLoading, isError, updateFilter, filter } = usePaginateAdoption()
    const { handleOpenModal } = useModal() || {}
    const router = useRouter()

    const columns: TableColumn<AdoptionResponse>[] = [
        { key: "id", label: "ID", render: (request) => `N-0${request.id}` },
        { key: "userName", label: "Usuario", render: (request) => `${request.requestAdoption.user.name} ${request.requestAdoption.user.lastName}` },
        { key: 'dni', label: 'DNI', render: (request) => request.requestAdoption.user.dni || '-' },
        { key: 'email', label: 'Correo', render: (request) => request.requestAdoption.user.email || '-' },
        { key: 'petName', label: 'Mascota', render: (request) => request.requestAdoption.pet.name },
        { key: 'status', label: 'Estado', render: (request) => <Badge color={returnStatus(request.status)}>{request.status}</Badge> },
        { key: 'createdAt', label: 'Fecha de solicitud', render: (request) => formatDateTime(request.createdAt) },
    ];
