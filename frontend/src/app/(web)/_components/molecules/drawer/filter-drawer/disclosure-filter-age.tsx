...
export default function DisclosureFilterAge() {
  ...
  const [sliderValues, setSliderValues] = useState<[number, number]>(() => {
    const minFilter = filters.find(
      (f) => f.feature === AGE_CONFIG.VALUE && f.name === AGE_CONFIG.MIN_ID,
    );
    const maxFilter = filters.find(
      (f) => f.feature === AGE_CONFIG.VALUE && f.name === AGE_CONFIG.MAX_ID,
    );

    return [
      minFilter ? parseInt(minFilter.id, 10) : MIN_AGE_DEFAULT,
      maxFilter ? parseInt(maxFilter.id, 10) : MAX_AGE_DEFAULT,
    ];
  });

  const currentMinFilter = filters.find(
    (f) => f.feature === AGE_CONFIG.VALUE && f.name === AGE_CONFIG.MIN_ID,
  );
  const currentMaxFilter = filters.find(
    (f) => f.feature === AGE_CONFIG.VALUE && f.name === AGE_CONFIG.MAX_ID,
  );

  const expectedMin = currentMinFilter
    ? parseInt(currentMinFilter.id, 10)
    : MIN_AGE_DEFAULT;
  const expectedMax = currentMaxFilter
    ? parseInt(currentMaxFilter.id, 10)
    : MAX_AGE_DEFAULT;

  if (sliderValues[0] !== expectedMin || sliderValues[1] !== expectedMax) {
    setSliderValues([expectedMin, expectedMax]);
  }
  ...
  const syncWithZustand = (min: number, max: number) => {
    ...
    const prevMin = currentFilters.find(
      (f) => f.feature === AGE_CONFIG.VALUE && f.name === AGE_CONFIG.MIN_ID,
    );
    const prevMax = currentFilters.find(
      (f) => f.feature === AGE_CONFIG.VALUE && f.name === AGE_CONFIG.MAX_ID,
    );
  ...
  const handleInputChange = (index: 0 | 1, value: string | number) => {
    const numValue =
      typeof value === "string"
        ? parseInt(value, 10) ||
          (index === 0 ? MIN_AGE_DEFAULT : MAX_AGE_DEFAULT)
        : value;
