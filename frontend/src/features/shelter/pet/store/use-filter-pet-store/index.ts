...
export const useFilterPetStore = create<FilterPetStore>((set) => ({
  ...
  confirmFilters: () => {
    ...
    const minAgeFilter = currentFilters.find(
      (f) => f.feature === AGE_CONFIG.VALUE && f.name === AGE_CONFIG.MIN_ID,
    );
    const maxAgeFilter = currentFilters.find(
    ...
    const confirmed: ConfirmedPetFilters = {
      gender: currentFilters
        .filter((f) => f.feature === FEATURE_FILTER.GENDER.VALUE)
        .map((f) => f.id),
      specieId: currentFilters
        .filter((f) => f.feature === FEATURE_FILTER.SPECIE.VALUE)
        .map((f) => f.id),
      size: currentFilters
        .filter((f) => f.feature === FEATURE_FILTER.SIZE.VALUE)
        .map((f) => f.id),
      breedId: currentFilters
        .filter((f) => f.feature === FEATURE_FILTER.BREED.VALUE)
        .map((f) => f.id),
  ...
  resetConfirmedFilters: () =>
    set({ confirmedFilters: initialConfirmedFilters }),
}));
