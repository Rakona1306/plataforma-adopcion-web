...
export default function DisclosureFilterBreed() {
  const {
    ...
    updateSearch,
    search,
  } = useGetPaginateBreed({ activeFeatures: [FEATURE_FILTER.SPECIE.VALUE] });
  const { removeFilter, setFilter, searchFilter, returnArrayString } =
    useFilterPlpStore();
  const [expanded, { toggle }] = useDisclosure(false);

  const localValues = returnArrayString(FEATURE_FILTER.BREED.VALUE);
                ...
                {hasNextPage && (
                  <Box w={"100%"} h={"auto"}>
                    <button
                      ...
                      className="text-sm! px-4 py-2 cursor-pointer hover:underline transition-all duration-300 leading-4 text-blue-700"
                    >
                      Mostrar mas
                    </button>
                  </Box>
