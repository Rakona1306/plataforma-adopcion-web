...
import { ReviewReqAdoptionDto, ReviewReqAdoptionSchema } from "../dto/dashboard/review-req-adoption";
...
export default function AdoptionReviewForm({ request }: Props) {
    ...
    const handleSubmit = (values: ReviewReqAdoptionDto) => {
        reviewAdoption({ requestId: request.id, dto: values }, {
            onSuccess: () => {
            ...
            onError: (error: any) => {
                if (error instanceof HttpError) {
                    console.log(error)
                    setError(error.message || 'Ocurrió un error al actualizar la solicitud de adopción.')
                }
            }
        })
    }

    console.log('RequestStatus Int', renderIntStatus(request.status, requestStatus));
