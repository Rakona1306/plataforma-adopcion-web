...
export default function useCreateRequestAdoption(
  props?: UseMutationOptions<void, unknown, CreateRequestAdoptionDto>,
) {
  ...
  const {
    mutate: createAdoption,
    isPending,
    isError,
  } = useMutation({
    ...props,
    mutationFn: (dto: CreateRequestAdoptionDto) =>
      requestAdoptionService.create(dto),
    onError: (error: any) => {
