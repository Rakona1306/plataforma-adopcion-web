...
interface IAdoptionFollowUpService {
  paginate(
    filter: AdoptionFollowUpFilter,
  ): Promise<Paginate<AdoptionFollowUpResponse>>;
...
class AdoptionFollowUpService implements IAdoptionFollowUpService {
  ...
  create(dto: CreateAdoptionFollowUp): Promise<void> {
    return this.httpClient.post(
      `${API_ENDPOINTS.ADOPTION_FOLLOW_UP.CREATE}`,
      dto,
    );
  }
  /*
  ...
  async paginate(
    ...
    return this.httpClient.get<Paginate<AdoptionFollowUpResponse>>(
      `${API_ENDPOINTS.ADOPTION_FOLLOW_UP.PAGINATE}`,
      { params: filter },
    );
  }
}
