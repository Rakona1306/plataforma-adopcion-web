...
export default function useValidateDni() {
    ...
    function checkDniWithAccount(name: string, lastName: string, dniData: ValidateDniResponse): boolean {

        const { nombres, apellidoPaterno, apellidoMaterno } = dniData.data;

        const nombreCoincide = normalizeText(name) === normalizeText(nombres);
        const apellidoCoincide = normalizeText(lastName) === normalizeText(`${apellidoPaterno} ${apellidoMaterno}`);
