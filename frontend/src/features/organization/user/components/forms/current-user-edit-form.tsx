...
function ContentForm({ user, onSubmit }: ContentProps) {
    ...
    async function handleSubmit(values: ChangeAccountInfoDto) {
        ...
        const isValid = await checkDniWithAccount(values.name, values.lastName, dniData)
    ...
    return (
        ...
        >
            {({ values }) => (
                <>
                    {validationError && (
                        <Alert color="red" title="Error de validación" withCloseButton onClose={() => setValidationError(null)}>
                            {validationError}
                        </Alert>
                    )}
                    ...
                    <Grid>
                        <Grid.Col span={{ base: 12, md: 6 }}>
                            <Input
                                ...
                                error={validationError && dniData && !checkDniWithAccount(user.name, user.lastName, dniData) ? "El nombre y/o apellido no coinciden con el DNI ingresado" : undefined}
                            />
                        </Grid.Col>
                        <Grid.Col span={{ base: 12, md: 6 }}>
                            <Input
                    ...
                    <Grid>
                        <Grid.Col span={{ base: 12, md: 6 }}>
                            <Button fullWidth type="submit" classNames={{
