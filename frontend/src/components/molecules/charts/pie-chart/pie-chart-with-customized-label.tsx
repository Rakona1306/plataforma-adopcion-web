...
const renderCustomizedLabel = ({ cx, cy, midAngle, innerRadius, outerRadius, percent, name, value }: PieLabelRenderProps) => {
    ...
    return (
        <>
            ...
            <text x={x} y={y + 15} fill="white" textAnchor={x > ncx ? 'start' : 'end'} dominantBaseline="central">
                {`${value}`}
            </text>
        </>
    );
...
export default function PieChartWithCustomizedLabel({ isAnimationActive = true, title }: Props) {
    return (
