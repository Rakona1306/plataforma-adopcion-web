...
export function SearchSelectField<T>({
    ...
    // Si el value cambia desde afuera (ej. reset del form), sincronizamos el texto
    useEffect(() => {
        if (selectedOption) {
            setSearch(String(selectedOption[displayField]));
        }
    ...
    const handleSearchChange = (value: string) => {
        ...
        if (
            !selectedOption ||
            String(selectedOption[displayField]) !== value
        ) {
            helpers.setValue("");
        }
              ...
              superpuesto; si no, el input de texto normal */}
                    {selectedOption && renderSelected ? (
                        ...
                        <TextInput
                            ...
                            leftSection={isLoading ? <Loader size="xs" /> : <SearchIcon width={20} />}
