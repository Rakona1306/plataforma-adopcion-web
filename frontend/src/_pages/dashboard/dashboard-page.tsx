...
export default function DashboardPage() {
  ...
  const renderBadge = (status: string) => {
    ...
    return (
      <span
        className={`px-2.5 py-1 rounded-full text-xs font-semibold tracking-wide ${styles[status] || "bg-gray-100"}`}
      >
        {status}
      </span>
    );
      ...
      <main className="flex-1 p-8 max-w-7xl mx-auto space-y-8 overflow-y-auto">
        <header className="border-b border-secondary/10 pb-4">
          ...
          <p className="text-sm text-slate-600">
            Monitoreo global del sistema de protección animal.
          </p>
        </header>
            ...
            <>
              <div className="px-6 py-4 bg-slate-50 border-b border-slate-100">
                <h3 className="font-bold text-slate-800">
                  Listado Global de Mascotas
                </h3>
              </div>
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50/50 border-b border-slate-100">
                    {["ID", "Nombre", "Especie", "Edad", "Estado"].map(
                      (h, i) => (
                        <th
                          key={i}
                ...
                <tbody className="divide-y divide-slate-100">
                  {MOCK_PETS.map((pet) => (
                    <tr
                      key={pet.id}
                      className="border-b border-slate-100 hover:bg-background/30 transition-colors"
                    >
