...
test.describe('Volunteer Page Acceptance Criteria', () => {
    ...
    test('CA1: Main title text and formatting requirements', async ({ page }) => {
    ...
    test('CA2: Responsive layout and alignment for benefits cards', async ({ page }) => {
        const viewport = page.viewportSize();
        const isMobile = viewport ? viewport.width < 768 : false;

        // Usamos las clases únicas de la grilla para evitar colisiones con otras secciones
        const benefitsGrid = page.locator('.grid.grid-cols-1.md\\:grid-cols-2.lg\\:grid-cols-4').first();
        const cardContent = benefitsGrid.locator('[data-slot="card-content"]').first();
    ...
    test('CA4: Contact form inputs are fully interactive', async ({ page }) => {
        ...
        await expect(messageInput).toHaveValue('Quiero ser voluntario en el albergue.');
    });

    test('CA5: Header section and CTA button are visible', async ({ page }) => {
        const subtitle = page.locator('h1 + p');
        await expect(subtitle).toBeVisible();
        await expect(subtitle).toHaveClass(/text-lg/);

        const ctaButton = page.locator('button').filter({ hasText: 'Comienza tu Aventura' });
        await expect(ctaButton).toBeVisible();
        await expect(ctaButton).toHaveClass(/font-bold/);
    });

    test('CA6: Contact information links are correctly formatted', async ({ page }) => {
        // Acotamos la búsqueda a la sección del formulario para evitar conflictos con el footer
        const contactSection = page.locator('section').filter({ hasText: '¿Listo para Hacer' });
        ...
        const whatsappElement = contactSection.getByRole('button', { name: 'Enviar Mensaje' })
